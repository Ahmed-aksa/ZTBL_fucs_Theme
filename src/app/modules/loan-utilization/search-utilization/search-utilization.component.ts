...
export class SearchUtilizationComponent implements OnInit {
    ...
    CheckEditStatus(loanUtilization: any) {
        ...
        if (this.isMCO) {
            if (
                loanUtilization.Status == 'P' ||
                loanUtilization.Status == 'R'
            ) {
                if (
                    loanUtilization.CreatedBy ==
                    this.loggedInUserDetails.User.UserId
                ) {
                    return true;
                } else {
    ...
    CheckViewStatus(loanUtilization: any) {
        if (this.isMCO) {
            if (
                loanUtilization.Status == 'C' ||
                loanUtilization.Status == 'S' ||
                loanUtilization.Status == 'A'
            ) {
                if (
                    loanUtilization.CreatedBy ==
                    this.loggedInUserDetails.User.UserId
    ...
    searchloanutilization() {
        ...
        this._loanutilizationService
            .searchUtilization(this._loanUtilizationSearch, this.userInfo)
            .pipe(
                finalize(() => {
                    this.loading = false;
                    this.spinner.hide();
            ...
            .subscribe((baseResponse) => {
                if (baseResponse.Success) {
                    this.dataSource.data =
                        baseResponse.LoanUtilization['Utilizations'];
    ...
    async LoadLovs() {
        this.loanutilizationStatusLov = await this._lovService.CallLovAPI(
            (this.LovCall = { TagName: LovConfigurationKey.UtilizationTypes })
        );
