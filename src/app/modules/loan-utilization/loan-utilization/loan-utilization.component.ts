...
} from 'app/shared/models/deceased_customer.model';
import {
    LoanUtilizationModel,
    UtilizationFiles,
} from 'app/shared/models/loan-utilization.model';
...
export class LoanUtilizationComponent implements OnInit {
    ...
    checkUser() {
        ...
        if (userInfo.User.userGroup[0].ProfileID == '56') {
            ...
            this.isBM = true;
            this.customerForm.controls.remarks.setValidators(
                Validators.required
            );
        }
    ...
    setMediaLimits() {
        this.MaxNumberOfImages = JSON.parse(
            localStorage.getItem('MaxNumberOfImages')
        );
        this.MaxNumberOfVideo = JSON.parse(
            localStorage.getItem('MaxNumberOfVideo')
        );
        this.VideoTimeLimit = JSON.parse(
            localStorage.getItem('VideoTimeLimit')
        );
    }
    ...
    createForm() {
        this.customerForm = this.fb.group({
            ...
            LoanDisbID: [
                this.loanUtilizationModel.LoanDisbID,
                Validators.required,
            ],
    ...
    onSelectFile(event) {
        if (this.images.length < this.MaxNumberOfImages) {
            if (event.target.files && event.target.files[0]) {
                ...
                if (Name != undefined) {
                    if (
                        ...
                        utilizationFile.file = Object.assign(
            ...
            this.layoutUtilsService.alertElement(
                '',
                'maximum ' + this.MaxNumberOfImages + ' Images allowed',
                ''
            );
            return;
        }
    ...
    onSelectFileV(event) {
        if (this.videos.length < this.MaxNumberOfVideo) {
            ...
            this.layoutUtilsService.alertElement(
                '',
                'maximum ' + this.MaxNumberOfVideo + ' Videos allowed',
                ''
            );
            return;
        }
    ...
    removeImage(url, val: number) {
        ...
        if (!url.includes('base64')) {
            this.imageid = this.images.find(
                (temp) => temp.ImageFilePath == url
            );
    ...
    removeVideo(url, val: number) {
        if (!url.includes('base64')) {
            this.videoid = this.videos.find(
                (temp) => temp.VideoFilePath == url
            );
    ...
    getDuration(e, i) {
        ...
        if (duration > this.VideoTimeLimit * 60) {
            this.layoutUtilsService.alertElement(
                '',
                'Duration cannot exceeded more then ' +
                    this.VideoTimeLimit +
                    ' minute',
                ''
            );
    ...
    previewImg() {
        const dialogRef = this.dialog.open(ViewFileComponent, {
            ...
            height: '90%',
            data: {
                documentView: this.DeceasedCustomerAttachedFile,
                url: this.imageUrl,
            },
        });
    }
    ...
    changeStatus(status: string) {
        if (
            status == 'S' &&
            (this.loanUtilizationModel.ID == undefined ||
                this.loanUtilizationModel.ID == null)
        ) {
            var msg = 'Please save before submitting';
            this.layoutUtilsService.alertElement('', msg, '');
            return;
        }
    ...
    find(val) {
        ...
        this._loanutilization
            ...
            .subscribe((baseResponse) => {
                if (baseResponse.Success) {
                    this.isEmpty = true;
                    this.DisbursementsRecoveries =
                        baseResponse.LoanUtilization['DisbursementsRecoveries'];
                    this.LoanApplicants =
                        baseResponse.LoanUtilization['LoanApplicants'];
                    this.Loanpurpose =
                        baseResponse.LoanUtilization['Loanpurpose'];
                } else {
                    this.layoutUtilsService.alertElement(
                        '',
                        baseResponse.Message,
            ...
            this._loanutilization
                ...
                .subscribe((baseResponse) => {
                    if (baseResponse.Success) {
                        ...
                        this.isEmpty = true;
                        var utilizationFiles =
                            baseResponse.LoanUtilization['UtilizationFiles'];
                        ...
                        if (utilizationFiles) {
                            for (let i = 0; i < utilizationFiles.length; i++) {
                                if (utilizationFiles[i].ImageFilePath) {
                                    ...
                                    this.imageUrl.push(
                                        utilizationFiles[i].ImageFilePath
                                    );
                                } else {
                                    this.videos.push(utilizationFiles[i]);
                                    this.videoUrl.push(
                                        utilizationFiles[i].VideoFilePath
                                    );
                                }
                            }
                        }
    ...
    OnChangeDisable(value) {
        if (value == '0') {
            ...
            this.customerForm.controls['DetailSourceIncome'].setValidators(
                Validators.required
            );
        ...
        this._loanutilization
            ...
            .subscribe((baseResponse) => {
                if (baseResponse.Success) {
                    ...
                    this.loanUtilizationModel.ID =
                        baseResponse.LoanUtilization.UtilizationDetail.ID;
                    console.log(
                        'id was saved here ' + this.loanUtilizationModel.ID
                    );
                    this.SaveImages();
                } else {
                    this.layoutUtilsService.alertElement(
    ...
    SaveImages() {
        if (this.currentIndex < this.images.length) {
            if (this.images[this.currentIndex].ImageFilePath == undefined) {
                ...
                this._loanutilization
                    .SaveMedia(
                        this.images[this.currentIndex].file,
                        this.loanUtilizationModel,
                        '0'
                    )
                    .pipe(
                        finalize(() => {
    ...
    SaveVideos() {
        ...
        if (this.currentIndex < this.videos.length) {
            if (this.videos[this.currentIndex].VideoFilePath == undefined) {
                ...
                this._loanutilization
                    .SaveMedia(
                        this.videos[this.currentIndex].file,
                        this.loanUtilizationModel,
                        '1'
                    )
                    .pipe(
