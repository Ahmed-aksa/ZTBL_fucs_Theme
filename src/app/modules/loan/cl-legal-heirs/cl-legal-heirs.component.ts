...
export class ClLegalHeirsComponent implements OnInit {
    ...
    createForm() {
        this.legalHeirsForm = this.formBuilder.group({
            ...
            LegalHeirsName: [
                this.legalHeirs.LegalHeirsName,
                [Validators.required],
            ],
    ...
    loadCustomers(CustomersLoanAppList) {
        this.loanDetail.CustomersLoanList =
            CustomersLoanAppList ?? of([CustomersLoanApp]);
    ...
    async LoadLovs() {
        ...
        this.GenderLov = await this._lovService.CallLovAPI(
            (this.LovCall = { TagName: LovConfigurationKey.Gender })
        );
    ...
    loadAppLegalHeirsDataOnUpdate(appLegalHeirsData, appCustomersLoanAppList) {
        ...
        for (var i = 0; i < appLegalHeirsData.length; i++) {
            ...
            var devProdFlag = this.RelationshipLov.filter(
                (x) => x.Id == appLegalHeirsData[i].RelationID
            );
    ...
    onSaveLegalHeirsForm() {
        ...
        this.legalHeirs.DobTxt = this.datePipe.transform(
            this.legalHeirs.Dob,
            'dd-MM-yyyy'
        );
        this.legalHeirs.Dob = this.datePipe.transform(
            this.legalHeirs.Dob,
            'ddMMyyyy'
        );
        this.legalHeirsForm.controls['Dob'].setValue(
            this.datePipe.transform(this.legalHeirsForm.value.Dob, 'ddMMyyyy')
        );
        console.log(this.legalHeirs);

        this.legalHeirs = Object.assign(
            this.legalHeirsForm.value,
            this.legalHeirsForm.getRawValue()
        );
        ...
        dialogRef.afterClosed().subscribe((res) => {
            ...
            if (this.legalHeirsArray.length == 0) {
                ...
                if (
                    ...
                    this.spinner.show();
                    this._loanService
                        .deleteLegalHeirs(legalHeir, this.legalHeirs.LoanAppID)
                        .pipe(
                            finalize(() => {
                                this.spinner.hide();
                            })
