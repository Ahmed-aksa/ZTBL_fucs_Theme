...
export class ClCustomersComponent implements OnInit {
    ...
    createForm() {
        this.loanCustomerForm = this.formBuilder.group({
            ...
            Relationship: [
                this.loanCustomer.RelationShip,
                [Validators.required],
            ],
        });
    }
    ...
    async LoadLovs() {
        ...
        var tempArray = await this._lovService.CallLovAPI(
            (this.LovCall = { TagName: LovConfigurationKey.AGPS })
        );
        this.AGPSLov = tempArray.LOVs;

        tempArray = await this._lovService.CallLovAPI(
            (this.LovCall = { TagName: LovConfigurationKey.Relationship })
        );
    ...
    searchCustomer() {
        ...
        var duplicateCustomer = this.customerArray.filter(
            (x) => x.cnic == this.loanCustomerForm.controls['CNIC'].value
        )[0];
        if (duplicateCustomer != undefined && duplicateCustomer != null) {
            this.layoutUtilsService.alertElement(
                '',
                'Customer CNIC Already Added',
                'Duplicate Cutomer'
            );
            return;
        }
        ...
        this._customerService
            ...
            .subscribe((baseResponse: BaseResponseModel) => {
                ...
                if (baseResponse.Success === true) {
                    ...
                    grid.agpsName = this.AGPSLov.filter(
                        (v) => v.Id == this.agpsModel
                    )[0].Name;
                    grid.Relationship = this.relationshipModel;
                    grid.RelationshipName = this.RelationshipLov.filter(
                        (v) => v.Id == this.relationshipModel
                    )[0].Name;
                    ...
                    Object.keys(this.loanCustomerForm.controls).forEach(
                        (key) => {
    ...
    loadAppCustomerDataOnUpdate(appCustomerData) {
        ...
        appCustomerData.forEach(function (item, key) {
            ...
            if (tempAgpsLov.length > 0) grid.agpsName = tempAgpsLov[0].Name;
            else grid.agpsName = item.Agps;
            //grid.Relationship = this.relationshipModel;

            var tempRealtionshipLov = tempRelationshipLovs.filter(
                (v) => v.Id == item.RelationID
            );
            if (tempRealtionshipLov.length > 0)
                grid.RelationshipName = tempRealtionshipLov[0].Name;
            else grid.RelationshipName = '';
    ...
    deleteRow(customerObj, index) {
        ...
        dialogRef.afterClosed().subscribe((res) => {
            ...
            if (this.customerArray.length == 0) {
                ...
                if (
                    customerObj.CustLoanAppID == null ||
                    customerObj.CustLoanAppID == undefined ||
                    customerObj.CustLoanAppID == ''
                ) {
            ...
            this._loanService
                ...
                .subscribe((baseResponse) => {
                    ...
                    if (baseResponse.Success) {
                        ...
                        cus.CustLoanAppID =
                            baseResponse.Loan.CustomersLoanApp.CustLoanAppID;
                        var addedCustomer = new CustomersLoanApp();
                        addedCustomer.CustLoanAppID =
                            baseResponse.Loan.CustomersLoanApp.CustLoanAppID;
                        addedCustomer.LoanAppID =
                            this.loanDetail.ApplicationHeader.LoanAppID;
