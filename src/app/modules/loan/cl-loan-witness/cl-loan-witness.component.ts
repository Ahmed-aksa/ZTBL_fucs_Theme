...
export class ClLoanWitnessComponent implements OnInit {
    ...
    onSavePersonalSuretiesForm() {
        ...
        this.personalSureties = Object.assign(
            this.personalSureties,
            this.personalSuretiesForm.getRawValue()
        );
        //this.personalSureties.LoanAppID = 0;
        this.personalSureties.SrNo = 0;
        this.personalSureties.LoanAppID =
    ...
    onSaveCorporateSuretiesForm() {
        ...
        this.corporateSureties = Object.assign(
            this.corporateSureties,
            this.corporateSuretiesForm.getRawValue()
        );
        //this.corporateSureties.LoanAppID = 0;
        this.corporateSureties.LoanAppID =
            this.loanDetail.ApplicationHeader.LoanAppID;
    ...
    onSaveLoanRefrencesForm() {
        ...
        this.loanRefrences = Object.assign(
            this.loanRefrences,
            this.loanRefrencesForm.getRawValue()
        );
        //this.loanRefrences.ReferenceID = 0;
        //this.loanRefrences.LoanAppID = 0;
        this.loanRefrences.LoanAppID =
            this.loanDetail.ApplicationHeader.LoanAppID;
        //this.loanRefrences.SrNo = 0;
        this.spinner.show();
        this._loanService
            .saveUpdateReferences(this.loanRefrences, this.loanDetail.TranId)
            .pipe(
                finalize(() => {
                    this.spinner.hide();
                })
    ...
    onSaveLoanWitnessForm() {
        ...
        this.loanWitness = Object.assign(
            this.loanWitness,
            this.loanWitnessForm.getRawValue()
        );
        //this.loanWitness.WitnessesID = 0;
        //this.loanWitness.LoanAppID = 0;
        this.loanWitness.LoanAppID =
            this.loanDetail.ApplicationHeader.LoanAppID;
        //this.loanWitness.SrNo = 0;
        this.spinner.show();
        this._loanService
            .SaveUpdateWitnesses(this.loanWitness, this.loanDetail.TranId)
            .pipe(
                finalize(() => {
                    this.spinner.hide();
                })
    ...
    onSaveLoanPastPaidForm() {
        ...
        this.loanPastPaid = Object.assign(
            this.loanPastPaid,
            this.loanPastPaidForm.getRawValue()
        );
        //this.loanPastPaid.PaidLoanID = 0;
        //this.loanPastPaid.LoanAppID = 0;
        this.loanPastPaid.LoanAppID =
            this.loanDetail.ApplicationHeader.LoanAppID;
        this.spinner.show();
        this._loanService
            .SaveUpdatePastPaidLoans(this.loanPastPaid, this.loanDetail.TranId)
            .pipe(
                finalize(() => {
                    this.spinner.hide();
                })
    ...
    onSaveCurrentLoansForm() {
        ...
        this.currentLoans = Object.assign(
            this.currentLoans,
            this.currentLoansForm.getRawValue()
        );
        //this.currentLoans.CurrentLoanID = 0;
        //this.currentLoans.LoanAppID = 0;
        this.currentLoans.LoanAppID =
            this.loanDetail.ApplicationHeader.LoanAppID;
        ...
        this.spinner.show();
        this._loanService
            .saveUpdateCurrentLoans(this.currentLoans, this.loanDetail.TranId)
            .pipe(
                finalize(() => {
                    this.spinner.hide();
                })
            ...
            this._loanService
                ...
                .subscribe((baseResponse) => {
                    ...
                    if (baseResponse.Success) {
                        this.loanDocumentsCheckList =
                            baseResponse.Loan.LoanDocumentCheckList;
                    }
                });
        } else {
    ...
    setUnsetDocumentCheckBox(docObj) {
        ...
        if (docObj.checked) {
            ...
            var docIndex = this.loanDocumentCheckListArray.findIndex(
                (x) => x.AppDocID == docObj.source.value
            );
    ...
    loadAppWitnessDataOnUpdate(
        appPersonalSureties,
        appReferences,
        appCorporateSureties,
        appLoanPastPaidList,
        appLoanWitnessList,
        appCurrentLoansList
    ) {
    ...
    onEditLoanCurrentList(CurrentLoanID) {
        for (var i = 0; i < this.editLoanCurrentList.length; i++) {
            if (this.editLoanCurrentList[i].CurrentLoanID == CurrentLoanID) {
                this.currentLoansForm.controls['BankName'].setValue(
                    this.editLoanCurrentList[i].BankName
                );
                this.currentLoansForm.controls['PurposeDetail'].setValue(
                    this.editLoanCurrentList[i].PurposeDetail
                );
                this.currentLoansForm.controls['TotalDebit'].setValue(
                    this.editLoanCurrentList[i].TotalDebit
                );
                this.currentLoansForm.controls['FundNonfundFlag'].setValue(
                    this.editLoanCurrentList[i].GurenteeDetail
                );
                this.currentLoansForm.controls['DateDebitAcheive'].setValue(
                    this.editLoanCurrentList[i].DateDebitAcheive
                );
                this.currentLoansForm.controls['AmountToPaid'].setValue(
                    this.editLoanCurrentList[i].AmountToPaid
                );
                this.currentLoansForm.controls['DueDate'].setValue(
                    this.editLoanCurrentList[i].DueDate
                );

                this.currentLoans.CurrentLoanID =
                    this.editLoanCurrentList[i].CurrentLoanID;
    ...
    onEditLoanWitnessArray(WitnessesID) {
        for (var i = 0; i < this.editLoanWitnessArray.length; i++) {
            if (this.editLoanWitnessArray[i].WitnessesID == WitnessesID) {
                this.loanWitnessForm.controls['Cnic'].setValue(
                    this.editLoanWitnessArray[i].Cnic
                );
                this.loanWitnessForm.controls['WitnessName'].setValue(
                    this.editLoanWitnessArray[i].WitnessName
                );
                this.loanWitnessForm.controls['WitnessAddress'].setValue(
                    this.editLoanWitnessArray[i].WitnessAddress
                );

                this.loanWitness.WitnessesID =
                    this.editLoanWitnessArray[i].WitnessesID;
            }
        }
    }
    ...
    onEditLoanPastPaidArray(PaidLoanID) {
        for (var i = 0; i < this.editLoanPastPaidArray.length; i++) {
            if (this.editLoanPastPaidArray[i].PaidLoanID == PaidLoanID) {
                this.loanPastPaidForm.controls['BankName'].setValue(
                    this.editLoanPastPaidArray[i].BankName
                );
                this.loanPastPaidForm.controls['AmountToPaid'].setValue(
                    this.editLoanPastPaidArray[i].AmountToPaid
                );
                this.loanPastPaidForm.controls['DueDate'].setValue(
                    this._common.stringToDate(
                        this.editLoanPastPaidArray[i].DueDate
                    )
                );
                this.loanPastPaidForm.controls['LastPaidDate'].setValue(
                    this._common.stringToDate(
                        this.editLoanPastPaidArray[i].LastPaidDate
                    )
                );

                this.loanPastPaid.PaidLoanID =
                    this.editLoanPastPaidArray[i].PaidLoanID;
            }
        }
    }
    ...
    onEditLoanCorporateSuretyArray(RefrenceNo) {
        for (var i = 0; i < this.editLoanCorporateSuretyArray.length; i++) {
            if (
                this.editLoanCorporateSuretyArray[i].CorporateSuretyID ==
                RefrenceNo
            ) {
                this.corporateSuretiesForm.controls['CompanyName'].setValue(
                    this.editLoanCorporateSuretyArray[i].CompanyName
                );
                var date = this.editLoanCorporateSuretyArray[i].MemorandumDate;
                let latest_date = this.datepipe.transform(date, 'yyyy-MM-dd');
                this.corporateSuretiesForm.controls['MemorandumDate'].setValue(
                    latest_date
                );
                this.corporateSuretiesForm.controls['RefrenceNo'].setValue(
                    this.editLoanCorporateSuretyArray[i].RefrenceNo
                );
                this.corporateSureties.CorporateSuretyID =
                    this.editLoanCorporateSuretyArray[i].CorporateSuretyID;
            }
        }
    }
    ...
    onEditPersonalSureties(PersonalSuretyID) {
        for (var i = 0; i < this.editLoanPersonalSuretiesArray.length; i++) {
            if (
                this.editLoanPersonalSuretiesArray[i].PersonalSuretyID ==
                PersonalSuretyID
            ) {
                //Personal Sureties Form

                this.personalSuretiesForm.controls['Cnic'].setValue(
                    this.editLoanPersonalSuretiesArray[i].Cnic
                );

                this.personalSuretiesForm.controls['FullName'].setValue(
                    this.editLoanPersonalSuretiesArray[i].FullName
                );
                this.personalSuretiesForm.controls['Percentage'].setValue(
                    this.editLoanPersonalSuretiesArray[i].Percentage
                );
                this.personalSuretiesForm.controls['Address'].setValue(
                    this.editLoanPersonalSuretiesArray[i].Address
                );
                this.personalSuretiesForm.controls['Phone'].setValue(
                    this.editLoanPersonalSuretiesArray[i].Phone
                );
                this.personalSuretiesForm.controls['AccountNo'].setValue(
                    this.editLoanPersonalSuretiesArray[i].AccountNo
                );
                this.personalSuretiesForm.controls['SourceofIncome'].setValue(
                    this.editLoanPersonalSuretiesArray[i].SourceofIncome
                );
                this.personalSuretiesForm.controls['AnnualIncome'].setValue(
                    this.editLoanPersonalSuretiesArray[i].AnnualIncome
                );
                this.personalSuretiesForm.controls['AssetValue'].setValue(
                    this.editLoanPersonalSuretiesArray[i].AssetValue
                );
                this.personalSuretiesForm.controls['PresentDues'].setValue(
                    this.editLoanPersonalSuretiesArray[i].PresentDues
                );
                this.personalSuretiesForm.controls['NetValue'].setValue(
                    this.editLoanPersonalSuretiesArray[i].NetValue
                );
                this.personalSureties.PersonalSuretyID =
                    this.editLoanPersonalSuretiesArray[i].PersonalSuretyID;
            }
        }
    }
    ...
    onEditRefrence(ReferenceID) {
        for (var i = 0; i < this.editLoanRefrenceArray.length; i++) {
            if (this.editLoanRefrenceArray[i].ReferenceID == ReferenceID) {
                this.loanRefrencesForm.controls['Cnic'].setValue(
                    this.editLoanRefrenceArray[i].Cnic
                );
                this.loanRefrencesForm.controls['Address'].setValue(
                    this.editLoanRefrenceArray[i].Address
                );
                this.loanRefrencesForm.controls['Name'].setValue(
                    this.editLoanRefrenceArray[i].Name
                );
                this.loanRefrencesForm.controls['Phone'].setValue(
                    this.editLoanRefrenceArray[i].Phone
                );
                this.loanRefrencesForm.controls['Fax'].setValue(
                    this.editLoanRefrenceArray[i].Fax
                );
                this.loanRefrencesForm.controls['Email'].setValue(
                    this.editLoanRefrenceArray[i].Email
                );
                this.loanRefrencesForm.controls['Ntn'].setValue(
                    this.editLoanRefrenceArray[i].Ntn
                );
                this.loanRefrences.ReferenceID =
                    this.editLoanRefrenceArray[i].ReferenceID;
                this.loanRefrences.SrNo =
                    this.editLoanRefrenceArray[i].LoanAppID;
            }
        }
    }
