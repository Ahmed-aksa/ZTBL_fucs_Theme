...
export class CalculateDbrComponent implements OnInit {
    ...
    onIncomeChange(e) {
        let index = this.dataSource.DBRIncomeList.findIndex(
            (inc) => inc.ID === e.srcElement.id
        );
        this.dataSource.DBRIncomeList[index].Value = e.srcElement.value;
    }
    onLiabChange(e) {
        let index = this.dataSource.DBRLiabilitiesList.findIndex(
            (inc) => inc.ID === e.srcElement.id
        );
    ...
    saveLoanDbr() {
        ...
        this._loanService
            ...
            .subscribe(
                (baseResponse: BaseResponseModel) => {
