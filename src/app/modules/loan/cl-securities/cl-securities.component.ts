...
export class ClSecuritiesComponent implements OnInit {
    ...
    async getSecurityType() {
        this.SecurityTypes = await this._lovService.CallLovAPI(
            (this.LovCall = { TagName: LovConfigurationKey.SecurityType })
        );
    ...
    searchSecurityType(securityTypeId) {
        ...
        if (
            securityTypeId != null &&
            securityTypeId != undefined &&
            securityTypeId != ''
        )
            this.SelectedSecurityType = this.SecurityTypes.LOVs.filter(
                (x) => x.Name.toLowerCase().indexOf(securityTypeId) > -1
            );
        else this.SelectedSecurityType = this.SecurityTypes.LOVs;
    }
    ...
    async getAreaQuantity() {
        this.areaQuantities = await this._lovService.CallLovAPI(
            (this.LovCall = { TagName: LovConfigurationKey.Quantity })
        );
    ...
    searchAreaQuantit(areaId) {
        ...
        if (areaId != null && areaId != undefined && areaId != '')
            this.SelectedAreaQuantities = this.areaQuantities.filter(
                (x) => x.Name.toLowerCase().indexOf(areaId) > -1
            );
        else this.SelectedAreaQuantities = this.areaQuantities;
    }
    ...
    createForm() {
        this.LoanSecuritiesForm = this.formBuilder.group({
            ...
            QuantityUnit: [
                this.loanSecurities.QuantityUnit,
                [Validators.required],
            ], //quantity area
    ...
    hasError(controlName: string, errorName: string): boolean {
        return this.LoanSecuritiesForm.controls[controlName].hasError(
            errorName
        );
    }
    ...
    getCustomerLand() {
        ...
        this._loanService
            .getCustomerLand(this.loanDetail.ApplicationHeader.LoanAppID)
            .pipe(finalize(() => {}))
            .subscribe((baseResponse) => {
                if (baseResponse.Success) {
                    this.customerLandList =
                        baseResponse.Loan.CustomersLoanLands;
    ...
    attachCustomerLandtoLoan() {
        ...
        if (this.customerLandList.length > 0) {
            ...
            this.layoutUtilsService.alertMessage(
                '',
                'Customer land details not found'
            );
        }
    }
    ...
    onSaveLoanSecurities() {
        ...
        this.loanSecurities = Object.assign(
            this.loanSecurities,
            this.LoanSecuritiesForm.getRawValue()
        );

        this.loanSecurities.Quantity = parseInt(
            this.LoanSecuritiesForm.controls['Quantity'].value
        );
        this.loanSecurities.UnitPrice = parseInt(
            this.LoanSecuritiesForm.controls['UnitPrice'].value
        );

        this.loanSecurities.PludgedValue =
            this.loanSecurities.Quantity * this.loanSecurities.UnitPrice;
        ...
        this._loanService
            .saveLoanSecurities(this.loanSecurities, this.loanDetail.TranId)
            .pipe(
                finalize(() => {
                    this.isSecuritiesFormInProgress = false;
                    this.spinner.hide();
            ...
            .subscribe((baseResponse) => {
                if (baseResponse.Success) {
                    ...
                    loanGrid.AppSecurityID = loanGrid.AppSecurityID =
                        this.loanSecurities.AppSecurityID;
                    loanGrid.BasisofMutation =
                        this.loanSecurities.BasisofMutation;
                    ...
                    loanGrid.MaxCreditLimit =
                        this.loanSecurities.MaxCreditLimit;
                    ...
                    Object.keys(this.LoanSecuritiesForm.controls).forEach(
                        (key) => {
    ...
    showUpdateSecuritiesForm(AppSecurityID) {
        for (var i = 0; i < this.editLoanSecuritiesArray.length; i++) {
            if (
                this.editLoanSecuritiesArray[i].AppSecurityID == AppSecurityID
            ) {
                console.log(
                    'This is AppSecurityID array',
                    this.editLoanSecuritiesArray[i]
                );

                this.LoanSecuritiesForm.controls['Quantity'].setValue(
                    this.editLoanSecuritiesArray[i].Quantity
                );
                this.LoanSecuritiesForm.controls['UnitPrice'].setValue(
                    this.editLoanSecuritiesArray[i].UnitPrice
                );
                this.loanSecurities.MaxCreditLimit =
                    this.editLoanSecuritiesArray[i].MaxCreditLimit;

                if (
                    (this.editLoanSecuritiesArray[i].CollTypeID != null,
                    this.editLoanSecuritiesArray[i].CollTypeID != undefined)
                ) {
                    var devProdFlag = this.SelectedSecurityType.filter(
                        (x) =>
                            x.Id == this.editLoanSecuritiesArray[i].CollTypeID
                    ); //[0].Id;
                    if (devProdFlag.length > 0) {
                        this.LoanSecuritiesForm.controls['CollTypeID'].setValue(
                            devProdFlag[0].CollTypeID
                        );
                    }
                }
                ...
                if (
                    ...
                    var devProdFlag = this.SelectedAreaQuantities.filter(
                        (x) =>
                            x.Name ==
                            this.editLoanSecuritiesArray[i].BasisofMutation
                    ); //[0].Id;
                    if (devProdFlag.length > 0) {
                        this.LoanSecuritiesForm.controls[
                            'QuantityUnit'
                        ].setValue(devProdFlag[0].Id);
                    }
                }
            }
        }
